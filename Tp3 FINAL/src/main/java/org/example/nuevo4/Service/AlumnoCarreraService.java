...
public class AlumnoCarreraService {
    ...
    public List<Ej2f_ResponseDTO> listarCarrerasPorCantidadInscriptos() {
        ...
        List<Ej2f_ResponseDTO> results = new ArrayList<>();
        for (Object[] row : this.alumnoCarreraRepository.listarCarrerasPorCantidadInscriptos()) {
            results.add(new Ej2f_ResponseDTO((Long) row[0], (String) row[1], (Long) row[2]));
        }
...
// Retorna todas las cerreras ordenadas alfabeticamente, con la cantidad de inscriptos y graduados por año, ordenado por año
    ...
    public List<Ej3DTOResultado> listaCarrerasInscriptosYGraduados() {
        ...
        List<Ej3DTOConsulta> listaInscriptos = new ArrayList<>();
        for (Object[] row : this. alumnoCarreraRepository.listaInscriptos()) {
            listaInscriptos.add(new Ej3DTOConsulta((String) row[0], (Integer) row[1], (Long) row[2]));
        }

        List<Ej3DTOConsulta> listaGraduados = new ArrayList<>();
        for (Object[] row : this. alumnoCarreraRepository.listaGraduados()) {
            listaGraduados.add(new Ej3DTOConsulta((String) row[0], (Integer) row[1], (Long) row[2]));
        }

        List<Ej3DTOResultado> listaResultado = new ArrayList<>();
        ...
        while (iteratorInscriptos.hasNext() && iteratorGraduados.hasNext()) {
            ...
            Integer anioInscriptos = elementoInscriptos.getAnio();
            Integer anioGraduados = elementoGraduados.getAnio();
    ...
    private List<Ej3DTOConsulta> convertirLista(List<Object[]> listaCarreras) {
        ...
        listaCarreras.forEach(a -> {
            Ej3DTOConsulta consulta = new Ej3DTOConsulta((String) a[0], (Integer) a[1], (Long) a[2]);
    ...
    private int comparar(Ej3DTOConsulta a, Ej3DTOConsulta b) {
        ...
        return Long.compare(a.getAnio(), b.getAnio());
